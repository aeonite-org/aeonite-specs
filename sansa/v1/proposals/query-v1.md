...
The evaluator slice is intentionally narrower than the full grammar. It covers execution of `from`, Boolean `where`, `order by`, `offset`, `limit`, and `select` over scalar literals including AEON toggle literal spellings (`yes`, `no`, `on`, `off`) and selected AEON scalar source literal families (`#hex`, `%radix`, `&encoding`, `^separator`, `!null`, and temporal-looking literals), resolution expressions, comparison expressions, Boolean expressions, membership expressions, string and number order keys, existence predicates over resolution expressions, cardinality predicates over resolved Binding Sets, built-in string functions, structured address activation with `path(...)`, missing-aware fallback with `fallback(...)`, dynamic direct-child resolution with `resolveChild(...)`, explicit reference following with `follow(...)`, concrete-value predicates, null predicates, special numeric predicates, and candidate-local projection expressions.
